...
void relocalizar_registros(packarch_t* pack, FILE* src, FILE* dest, size_t umbral) {
	...
	for (size_t i = 0; i<tamanio_bloque(pack); i++) if (bitmap[i] == 1) bmax = i+1;
...
bool unificar(size_t umbral, size_t chunk, packarch_t* pack, char* carpeta) {
	...
	while (original>2) {
		...
		viejo = vector_crear(free);
		for(size_t i = 0; i<vector_obtener_cantidad(nuevo);	i++) vector_guardar(viejo,vector_obtener(nuevo,i));
...
packarch_t* inicializar () {
	printf ("Introduzca el archivo sobre el que trabajar\n");
	char* arch = leer_linea();
	printf ("Introduzca el valor de n\n");
	char* n = leer_linea();
	printf ("Introduzca el formato de registro\n");
	char* registro = leer_linea ();
	packarch_t* archivo = crear_packarch (arch, strtonum(n), registro);
	free(arch);
	free(n);
	free(registro);
	return archivo;
...
int main () {
	packarch_t* archivo = inicializar();
	printf ("\n1-Exportar a CSV\n2-Relocalizar Registros\n3-Reordenar Registros\n4-Salir\n\n");
	char* linea = leer_linea ();
	if (linea && strcmp(linea,"4") != 0) {
		if (strcmp(linea,"1") != 0) {
			if (strcmp(linea,"2") != 0) {
				if (strcmp(linea,"3") != 0) printf ("ERROR DE COMANDO");
					else {
						printf("Ingrese el umbral minimo deseado (digito porcentual)\n");
						char* umbral = leer_linea ();
						printf("Ingrese el numero de chunks deseado\n");
						char* chunk = leer_linea();
						printf("Ingrese la carpeta de archivos temporales\n");
						char* carpeta = leer_linea();
						reordenar (archivo,strtonum(umbral),strtonum(chunk),carpeta);
						free(umbral);
						free(chunk);
						free(carpeta);
					}	
					...
					printf("Ingrese el umbral minimo deseado (digito porcentual)\n");
					char* umbral = leer_linea ();
					relocalizar (archivo,strtonum(umbral));
					free(umbral);
				}
